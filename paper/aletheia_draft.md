...
The nine signatures we identify are not primarily a benchmark contribution. They are the components of a *missing infrastructure layer* — the AI behavioral equivalent of what OpenTelemetry provides for distributed systems: a standard instrumentation model, a taxonomy of signals to collect, and a set of alerting primitives. The central question is structural: *what are the fundamental interfaces through which an AI system can fail, and how do we instrument them?* Section 2 answers the first part. Sections 3–5 provide the empirical validation. Section 6 describes the production architecture.
...
**Formal completeness argument.** We claim that these eight interfaces are *currently sufficient* — empirically complete with respect to the 2,571-entry analyzed corpus — and that any newly proposed signature must demonstrate a genuinely new interface to warrant inclusion. The argument: any behavioral failure of an AI system in deployment can be characterized by asking which of the following questions the system answered incorrectly: (1) Is what I am saying true? (2) Is this person who they claim to be? (3) Is this instruction safe to follow? (4) Am I staying within my authorized scope? (5) Do I understand what is actually being asked? (6) Do I know the limits of my own knowledge? (7) Do I understand who I am talking to? (8) Am I evaluating this information independently of its claimed source? (9) Am I detecting accumulating harm signals in my own outputs?

If a proposed new failure mode cannot be characterized by one of these nine questions, it represents a genuinely new interface and warrants a new signature. We evaluated several candidate additions against this test:

- *Multi-turn memory drift* (the system's behavior changes over long conversations in ways inconsistent with its instructions): reduces to repeated S3 (scope expansion) or S8 (no feedback correction) depending on whether the drift is task-boundary or output-quality related.
- *Value lock-in* (the system refuses to update beliefs in response to new evidence): reduces to S1 (confidence without grounding) when the belief is factual, or S4 (context blindness) when the evidence is pragmatic.
- *Goal misgeneralization* (the system pursues a learned proxy goal rather than the intended goal in a new environment): reduces to S3 (scope creep beyond mandate) in operational terms.

A clarification on scope: these reductions are *operational*, not architectural. Goal misgeneralization, value lock-in, and multi-turn memory drift have deep architectural roots in training dynamics and reward modeling that behavioral telemetry cannot fix. Aletheia detects the *symptom* at the interface — the observable behavioral manifestation — not the underlying cause. This is by design: the same is true of distributed systems telemetry, which alerts on elevated error rates without diagnosing the root cause in application code. Observability and root-cause remediation are complementary, not competing, engineering functions.

The 2,571-entry cross-corpus dataset provides empirical support for completeness: after excluding entries that describe infrastructure failures (out of scope), economic harms (out of scope), and geopolitical risks (out of scope), the remaining entries map to one or more of the nine signatures without producing a systematic residual category. The 1,165 MIT Risk entries that did not match any signature describe risks (e.g., "AI-driven economic concentration," "AI military applications") that are societal-scale policy failures, not behavioral failures of a deployed AI system — the boundary Aletheia is built on.

**S4 and the documentation gap problem.** Context Blindness (S4) has the weakest incident corpus representation: zero AIID matches via keyword classification, three MIT Risk entries, forty hand-curated cases. A reviewer might reasonably ask whether S4 should be included at all. We argue it must be, on two grounds. First, structural: removing S4 leaves the meaning–intent interface unmonitored — a gap that is architecturally indefensible regardless of current corpus coverage. A distributed systems observability framework that omitted memory-leak detection because early incident databases rarely logged it would be considered incomplete, not conservative. Second, empirical: the hand-curated cases demonstrate that S4 failures exist, are distinct from other signatures, and are measurable with a designed benchmark. The absence from incident databases reflects a *documentation bias* — pragmatic failures (misread sarcasm, missed subtext, literal interpretation of idioms) are rarely reported as discrete incidents because their harms are diffuse and hard to attribute — not an absence of the failure mode itself. As AI assistants proliferate in high-stakes communication contexts (legal drafting, clinical communication, mental health support), S4 documentation will accumulate. The taxonomy should not be limited to what current databases happen to capture.
...
**Why keyword classification rather than embeddings or LLM judges?** Three properties of this task make keyword classification the correct choice. First, *interpretability*: a keyword match is fully auditable — an operator can inspect exactly which term triggered a classification. This is a non-negotiable property for a framework intended to function as production telemetry, where false positives have operational consequences. A neural classifier that returns a probability score without an explanation cannot be deployed in this role. Second, *reproducibility at training-set size*: with 100–400 labeled examples per signature, a fine-tuned embedding classifier would overfit and would not generalize across the vocabulary registers (journalist prose, ML evaluation language, academic risk literature) that the cross-corpus validation requires. Keyword classifiers generalize across registers by design — the vocabulary list is the generalization. Third, *production deployability*: the same keyword rules that annotate incidents can be deployed as runtime telemetry rules in an API gateway with no GPU dependency and sub-millisecond latency. This is the architecture described in Section 6.2.

LLM judges (using a language model to classify incidents) were explicitly rejected on two grounds: self-preference bias and circular validation. Self-preference bias is well documented (Panickssery et al., 2024): LLM judges systematically favor outputs from their own model family. Using an LLM to judge whether Claude, GPT-4o, and Gemini exhibit failure signatures would introduce systematic bias against whichever model is most similar to the judge. Circular validation: using a language model to validate a benchmark that measures language model behavior means that the validation methodology has the same failure modes as the thing being measured. We use a methodology — keyword matching against human-written incident reports — that is architecturally independent of the models being evaluated.

**Precision and recall.** We validate the scorer against a 25-case ground-truth suite covering all eight signatures — curated known-positive responses (model exhibiting the failure mode) and known-negative responses (model handling correctly), with expected labels defined independently of the scorer. Results: 100% precision, 100% recall, F1 = 1.00 across all signatures (25/25 cases, 0 false positives, 0 false negatives). This replaces the prior estimated 15–20% FP rate with a measured result on a structured validation set. As detailed in Section 6.3, a 200-incident population-level holdout with independent annotators remains planned to establish precision/recall curves under realistic corpus sampling conditions. High recall at the cost of some precision is the correct design trade-off for incident corpus construction: missed incidents are permanent losses; false positives can be filtered in subsequent review rounds.
...
**Classifier thresholds:** The keyword classifier assigns 0.4 confidence per keyword match, capped at 1.0. A confidence score at or above 0.3 is required to label an incident as exhibiting a signature. Under this scheme, a single keyword match (0.4) is sufficient to cross threshold; two or more matches yield 0.8 or higher. This means a single ambiguous term can in principle trigger classification — a deliberate design choice that prioritizes recall over precision. The scorer was validated against a 25-case ground-truth suite (see §4.2) achieving 100% precision and recall; threshold sensitivity analysis (0.3 vs 0.5) is deferred to the planned 200-incident population-level holdout described in Section 6.3. The full keyword vocabulary is listed in Appendix C.
...
**Stateful multi-turn tracking.** Single-turn payload inspection is sufficient for S1, S2a, S2b, S3, S4, S5, and S7, where the failure is detectable within one response. S6 (Vulnerability Signal Blindness) and S8 (Feedback Loop Absence) are different: both require tracking conversation state across turns. A user's distress signal (S6) may be distributed across several messages; a feedback loop (S8) only becomes detectable as a pattern of accumulating outputs. The proxy layer must therefore maintain a rolling context window — the last N turns keyed by session ID — to evaluate these two signatures. In practice this requires a lightweight session state store (e.g., Redis with a TTL matching the session timeout) that the gateway can read and write with negligible latency overhead. The session token is passed as a request header; the proxy uses it to retrieve and update the conversation window without storing message content beyond the detection window. This is architecturally identical to the session-keyed state stores used in API rate limiting and distributed fraud detection, and is the principal engineering distinction between stateless single-turn guardrails and a full behavioral telemetry layer.
...
**Keyword classifier:** The incident classifier uses keyword heuristics that may miss incidents described in unusual terminology and may over-include superficially similar incidents. The current threshold of 0.3 permits classification on a single keyword match (confidence 0.4), prioritizing recall over precision. To validate scorer accuracy, we constructed a 25-case ground-truth suite covering all eight signatures: curated known-positive responses (models exhibiting the failure) and known-negative responses (models handling correctly), with expected labels defined independently of the scorer. The scorer achieves 100% precision and 100% recall on this suite (25/25 cases, 0 false positives, 0 false negatives), with per-signature F1 = 1.00 across all signatures. This result is based on a hand-curated validation set rather than a sampled incident corpus; a formal 200-incident holdout validation with independent annotators remains planned for a follow-on study to establish population-level precision/recall curves and evaluate threshold sensitivity.
...
**S2b benchmark:** S2b (Adversarial Input Exploitation) has 376 validated incidents in the cross-corpus dataset — the third-largest signature by prevalence. The scorer has been significantly strengthened with five detection layers covering modern evasion patterns: explicit role adoption, dual-response format ([Normal]/[Developer]), fictional-frame compliance, logical inversion tricks, and encoded/obfuscated harmful output. Preliminary detection rates at n=5 probes show 40% for Llama 3.2, consistent with known open-weight jailbreak susceptibility. Full n=100 benchmark runs against frontier models are the highest-priority addition to the next evaluation cycle, requiring careful probe design to distinguish direct jailbreak sequences from indirect prompt injection.
