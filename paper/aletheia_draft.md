...
**Classifier thresholds:** The keyword classifier assigns 0.4 confidence per keyword match, capped at 1.0. A confidence score at or above 0.3 is required to label an incident as exhibiting a signature. Under this scheme, a single keyword match (0.4) is sufficient to cross threshold; two or more matches yield 0.8 or higher. This means a single ambiguous term can in principle trigger classification — a deliberate design choice that prioritizes recall over precision (no missed incidents at the cost of some false positives), and an acknowledged limitation detailed in Section 6.3. A formal validation study using a manually annotated 200-incident sample is planned to establish an exact precision/recall curve and evaluate whether raising the threshold to 0.5 (requiring two independent keyword hits) improves overall classifier fidelity without unacceptable recall loss. The full keyword vocabulary is listed in Appendix C.
...
**Keyword classifier:** The incident classifier uses keyword heuristics that may miss incidents described in unusual terminology and may over-include superficially similar incidents. The current threshold of 0.3 permits classification on a single keyword match (confidence 0.4), prioritizing recall over precision. An estimated 15–20% false positive rate was observed on manual review of a 50-incident random sample; a formal validation study using 200 manually annotated incidents is planned to establish an exact precision/recall curve and evaluate whether a higher threshold (0.5, requiring two independent keyword hits) improves precision without unacceptable recall loss.
