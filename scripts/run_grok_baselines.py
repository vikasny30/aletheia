"""
Run all 9 Aletheia signatures against Grok (xAI) and print results.
Uses the private probe corpus from aletheia-probes/.
Usage: python3 scripts/run_grok_baselines.py [model] [n_probes]
...
MODEL = sys.argv[1] if len(sys.argv) > 1 else "grok-3-mini"
...
for sig_id in ALL_SIGNATURE_IDS:
    probes = private_prompts.SIGNATURES[sig_id]["probes"][:N]
    t0 = time.time()
    ...
    print(f"  {sig_id}  {r['detection_pct']:5.1f}%  CI95={ci:<16} {elapsed:5.1f}s{flag}{errors}", flush=True)
